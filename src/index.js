...
const buttons = [
  ...
  {
    addCard: content.querySelector(".profile__add-button"),
    name: popupKeys.addCard,
  },
  {
    updateAvatar: content.querySelector(".profile__image-button"),
    name: popupKeys.updateAvatar,
  },
];
const popups = {
 ...
 */
const getCardElement = (data, userId) =>
  renderCard(data, userId, {
    deleteCard: (event) => initDeleteCard(event, data._id),
    likeCard: (event, likeCounter) =>
      handleToggleLike(event, data._id, likeCounter),
    openGallery: () => handleOpenPopup(popupKeys.gallery, data),
  });

// Открытие попапов
buttons.forEach(({ name, ...item }) =>
  item[name].addEventListener("click", () => handleOpenPopup(name))
);
...
popups.deleteCard
  .querySelector(popupConfig.submitButtonSelector)
